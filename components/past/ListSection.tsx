...
import PastRow, { ROW } from "./PastRow";
import type { AllPast } from "./useAllPast";

/**
 * Every past lake, largest first. The largest few arrive with the page; the rest load when
    ...
    <section id="list" aria-labelledby="list-title" className="page-x pb-16 md:pb-26 scroll-mt-6">
      <div className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-2">
        <h2 id="list-title" className="font-serif text-[26px] md:text-[36px] leading-none">
          <span className="highlight">Every past lake</span>
        </h2>
        {filtered && (
      ...
      {shown.length > 0 && (
        <>
          <div aria-hidden className={`${ROW} mt-6 hidden md:grid pb-2 border-b border-rule label text-missing`}>
            ...
            <span className="col-span-3">When it went</span>
            <span className="col-span-4">Why</span>
          </div>
          <ul className="mt-6 md:mt-0 border-t border-rule md:border-t-0">
            {shown.map((lake) => (
              <PastRow key={lake.id} lake={lake} printed={printed[lake.id]} />
            ))}
          </ul>
        </>
      )}
      ...
      {rows === null && !filtered && all.status !== "error" && (
        <button
          type="button"
          onClick={onLoad}
